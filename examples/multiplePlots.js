var multiplePlotsSketch = function(p) {
	...
	p.setup = function() {
		...
		for ( i = 0; i < 500; i++) {
			points1a[i] = new GPoint(i, p.noise(0.1 * i) + 1);
			points1b[i] = new GPoint(i, p.noise(500 + 0.1 * i) + 0.5);
			points1c[i] = new GPoint(i, p.noise(1000 + 0.1 * i));
		}
		...
		console.log('gaussianStack.length',gaussianStack.length);
