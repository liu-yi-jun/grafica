...
function GAxis(parent, type, dim, lim, log) {
	...
	this.isPlotTicks = true;
	this.tickLabelOffset = 7;
	this.ticksPrecision = undefined;

	this.drawFirstAndLastOnly = false;
...
GAxis.prototype.formatNumber = function(num) {
	if (typeof num === 'string') {
        var numericNum = parseFloat(num);
        if (isNaN(numericNum)) {
            return num;
        }
        num = numericNum;
	...
	if (num >= -1000 && num <= -0.001 || num >= 0.001 && num <= 1000) {
		return num.toString();
	  }
...
GAxis.prototype.drawAsYAxis = function() {
	...
	if (this.drawTickLabels) {
		...
		if (this.rotateTickLabels) {
			...
			for ( i = 0; i < this.plotTicks.length; i++) {
					 // 如果只绘制第一个和最后一个刻度
				if (this.drawFirstAndLastOnly && (i === 0 || i === this.plotTicks.length - 1)) {
					if (this.ticksInside[i] && this.tickLabels[i] !== "") {
						this.parent.push();
						if(i === 0) {
							this.parent.translate(-this.offset - this.tickLabelOffset, this.plotTicks[i] - 3);	
						}else if (i === this.plotTicks.length - 1) {
							this.parent.translate(-this.offset - this.tickLabelOffset, this.plotTicks[i] + 3);
						}
						
						// this.parent.rotate(-halfPI);
						this.parent.text(this.formatNumber(this.tickLabels[i]), - this.tickLabelOffset, 0);
						this.parent.pop();
					}
					continue;
				} else if (!this.drawFirstAndLastOnly) {
						if(this.plotTicks.length >=6 && !this.showLastTick && (i === 0 || i === this.plotTicks.length - 1)) {
							continue;
						}
						if (this.ticksInside[i] && this.tickLabels[i] !== "") {
							this.parent.push();
...
GAxis.prototype.setShowLastTick = function(showLastTick) {
	this.showLastTick = showLastTick;
};
...
GAxis.prototype.setDrawPlotTicks = function(isPlotTicks) {
	this.isPlotTicks = isPlotTicks;
};
...
// 设置只绘制第一个和最后一个tick
GAxis.prototype.setDrawFirstAndLastOnly = function(drawFirstAndLastOnly) {
	this.drawFirstAndLastOnly = drawFirstAndLastOnly;
};
...
// 新增的返回plotPoints
GLayer.prototype.getPlotPoints = function() {
...
// 新增的返回plotPoints
GPlot.prototype.getPlotPoints = function() {
	if (arguments.length === 1) {
		return this.getLayer(arguments[0]).getPlotPoints();
	} else if (arguments.length === 0) {
		return this.mainLayer.getPlotPoints();
	} else {
		throw new Error("GPlot.getPlotPoints(): signature not supported");
	}
};
